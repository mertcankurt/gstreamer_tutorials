...
class GstClass:
    ...
    def __post_init__(self):
        ...
        if not self.pipeline or not self.source or not self.audio_convert or not self.audio_resample or not self.audio_sink:
            logger.error("Not all elements could be created.")
            sys.exit(1)

        # Build the pipeline. Note that we are NOT linking the source at this point.
        # We will do it later.
        self.pipeline.add(self.source, self.audio_convert, self.audio_resample, self.audio_sink)
        if not self.audio_convert.link(self.audio_resample) or not self.audio_resample.link(self.audio_sink):
